...
public class TodosController {
    ...
    public HttpEntity<Collection<Todo>> listAll() {
        List<Todo> resourceWithUrls = todos.stream().map(todo -> todo).collect(Collectors.toList());
        return new ResponseEntity<>(resourceWithUrls, OK);
    }

    @RequestMapping(value = "/{todo-id}", method = GET)
    public HttpEntity<Todo> getTodo(@PathVariable("todo-id") long id) {
...
//    }
    ...
    private HttpEntity<Todo> respondWithResource(Todo todo, HttpStatus statusCode) {
//        ResourceWithUrl resourceWithUrl = toResource(todo);

        return new ResponseEntity<>(todo, statusCode);
    }
}