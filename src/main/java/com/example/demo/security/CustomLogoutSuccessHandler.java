  ...
  public void onLogoutSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
    ...
    if (token != null && token.startsWith(BEARER_AUTHENTICATION)) {
      ...
      if (oAuth2AccessToken != null) {
        ...
        //                boolean authRemoved = ((DefaultTokenServices) authorizationServerTokenServices).revokeToken
        // (tokenValue); // <- true
        //                boolean resourceRemoved = ((DefaultTokenServices) resourceServerTokenServices).revokeToken
        // (tokenValue); // <- true
